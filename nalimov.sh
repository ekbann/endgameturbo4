...
#
# Free version of ChessBase Endgame Turbo 4 (120GB of Nalimov Tablebases)
...
#	res=`grep "$i" nalimov/all-33.md5 | awk '{print "http://tablebase.sesse.net/3+3/"$2}' | xargs wget --spider -O - 2>&1 | grep "Length:" | awk '{sum+=$2;} END {print int(sum/1024/1024);}'`
#	echo -e "\t\t$res MB" 
#	grandtotal=$(($grandtotal + $res))

	filelist+=$(grep "$i" nalimov/all-33.md5 | awk '{print "http://tablebase.sesse.net/3+3/"$2}')
	filelist+=$'\n' 
done
...
do
#	echo -e "\tchecking download size for: $i"
#	res=`grep "$i" nalimov/all-33p.md5 | awk '{print "http://tablebase.sesse.net/3+3-with-pawns/"$2}' | xargs wget --spider -O - 2>&1 | grep "Length:" | awk '{sum+=$2;} END {print int(sum/1024/1024);}'`
#	echo -e "\t\t$res MB"
#	grandtotal=$(($grandtotal + $res))

	filelist+=$(grep "$i" nalimov/all-33p.md5 | awk '{print "http://tablebase.sesse.net/3+3-with-pawns/"$2}')
	filelist+=$'\n' 
done
...
do
#	echo -e "\tchecking download size for: $i"
#	res=`grep "$i" nalimov/all-42p.md5 | awk '{print "http://tablebase.sesse.net/4+2-with-pawns/"$2}' | xargs wget --spider -O - 2>&1 | grep "Length:" | awk '{sum+=$2;} END {print int(sum/1024/1024);}'`
#	echo -e "\t\t$res MB"
#	grandtotal=$(($grandtotal + $res))

	filelist+=$(grep "$i" nalimov/all-42p.md5 | awk '{print "http://tablebase.sesse.net/4+2-with-pawns/"$2}')
	filelist+=$'\n' 
done
